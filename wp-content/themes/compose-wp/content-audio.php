...
?>
	...
	<article id="post-<?php the_ID(); ?>" <?php post_class('col-xs-12 compose-blog-post clearfix'); ?>>
		...
		<header class="entry-header">
			<?php
				if ( is_single() ) :
					the_title( '<h2 class="entry-title"><i class="fa fa-microphone"></i> ', '</h2>' );
				else :
					the_title( '<h2 class="entry-title"><a href="' . esc_url( get_permalink() ) . '" rel="bookmark"><i class="fa fa-microphone"></i> ', '</a></h2>' );
				endif;
			?>
		</header><!-- .entry-header -->
